...
class SketchCanvasView @JvmOverloads constructor(
    ...
    interface Listener {
        ...
        fun onPartTapped(part: SketchPart)
        fun onPartsChanged()
    }
    ...
    private val gestureDetector = GestureDetector(context, object : GestureDetector.SimpleOnGestureListener() {
        override fun onLongPress(e: MotionEvent) {
            if (hitTest(e.x, e.y) == null) listener?.onLongPressEmptySpace(e.x, e.y)
        }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (parts.isEmpty()) {
            canvas.drawText(
                "Long-press anywhere to start sketching",
                width / 2f,
                height / 2f,
                hintPaint,
            )
